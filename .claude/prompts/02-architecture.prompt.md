...
## Input
`.claude/artifacts/<TICKET-ID>/requirements.md` (written in Phase 1).
...
4. Write `.claude/artifacts/<TICKET-ID>/architecture.md` with:
   - Overview paragraph
...
5. Commit: `git add .claude/artifacts/<TICKET-ID>/architecture.md && git commit -m "docs(<TICKET-ID>): Phase 2 architecture"`
