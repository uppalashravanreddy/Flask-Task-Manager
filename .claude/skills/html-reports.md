...
This script reads the three individual reports, extracts pass/fail counts, and writes a `.claude/reports/<TICKET-ID>/index.html` with:
...
```bash
# Windows
start .claude/reports/<TICKET-ID>/index.html
# Or open in browser:
python -m http.server 8080 --directory .claude/reports/<TICKET-ID>/
...
After generating reports, create/update the `<TICKET-ID> Test Results` Confluence page (see `confluence-sync` skill) with a summary table and a note that full HTML reports are available locally at `.claude/reports/<TICKET-ID>/index.html`.
